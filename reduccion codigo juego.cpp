// Juego-javier-.cpp : Este archivo contiene la función "main". La ejecución del programa comienza y termina ahí.
//
...
bool checkEnemyStatus1(int&enemyHP,enemyName ) {
    if (enemyHP <= 0) {
...
int heroAttackEnemy1(int&damage,int&enemyName,int&enemyHP) {
    cout << "Atacas al enemigo " << enemyName << " y le haces un danyo de " << damage << " \n";
    return enemyHP - damage;
}


void enemyAttack1(int&damage,int&heroHP,int&enemyName,int&heroIsAlive) {
...
int main()
{
    ...
    while (heroIsAlive && (enemyIsAlive1 || enemyIsAlive2)) {
        ...
        if (enemyChoosed == 1) {
            ...
            enemyHP2 = heroAttackEnemy1(heroDamage);
            enemyIsAlive2 = checkEnemyStatus1();
        }
